...
async def generate_prompt(llm_model, data_path, filename, recording_id):
    ...
    for i, query in enumerate(query_list):
        ...
        for result in results_list:
            ...
            # Invoke the model with the formatted prompt and collect the response
    ...
    *Feedback on the Jamie platform:*
    ...
    Instructions:
        ...
        3) Point Must be have more descriptive with related keyword in summary.
        4) Format the final summary as a Python list, with each point as a separate list item.
        5) Ensure the final summary is professional and clearly organized.
        6) Give me each section is detailed and well-organized, using dash (-) points for clarity. if have point have a title then wrap title with single astrict (*). 
...
# Function to extract task and decision content from a Action Item using regular expressions
def extract_task_and_decision_content(text):
    # print("Action Item ", text)
...
def callback_mom(llm_model, DATA_PATH, filename, recording_id):
    print("callback_mom called for ", recording_id)
    start_time = time()
    response = asyncio.run(generate_prompt(llm_model, DATA_PATH, filename, recording_id))
