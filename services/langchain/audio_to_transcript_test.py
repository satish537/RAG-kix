...
async def extract_audio_from_video(video_path, output_audio_path):
    try:
        ...
        if file_extension in ['.mp4', '.mp3']:
            # Ensure the output audio path has a unique name
            ...
            print("Unsupported file format. Please use MP4, MP3, or WAV.")
            return None
...
async def process_transcript_test(whisperx_model, data_path, audio_path, recording_id):
    ...
    try:
        ...
        merged_output = {
            "plain_text_transcription": plain_text,
            "json_word_transcription": json_transcription
        }
