...
async def process_result_group(result_group, context, theme, description, projectID, chatId, callbackUrl=None):
...
async def process_background_results(other_results, theme, description, projectID, chatId, callbackUrl):
    ...
    try:
        ...
        full_context = "\n\n---\n\n".join([doc.page_content for doc, _ in other_results])
        
        for idx, group in enumerate(other_result_groups):
            print(f"Processing background group {idx+1}/{len(other_result_groups)}")
            await process_result_group(group, full_context, theme, description, projectID, chatId, callbackUrl)
...
async def generate_quotes_details(projectID: str, questionId, participantId, theme, description, metadata, kValue, chatId, callbackUrl, percentage: float = 50):
    ...
    results = db.similarity_search_with_score(prompt, k=10, filter=filter_obj)
    ...
    full_context = "\n\n---\n\n".join([doc.page_content for doc, _score in initial_results])
    ...
    with ThreadPoolExecutor(max_workers=math.ceil(len(initial_results)/2)) as executor:
        ...
        futures = [
            asyncio.create_task(process_result_group(group, full_context, theme, description, projectID, chatId))
