...
async def generate_theme_details(projectID: str, prompt: str, questionId, participantId, kValue: int):
    ...
    if questionId and participantId:
        results = db.similarity_search_with_score(prompt, k=kValue, filter={"$and": [{"questionId": questionId}, {"participantId": participantId}]})
    else:
    ...
    print(f"Total number of results is {len(results)}")
    results_list = await split_list_into_parts(results, 4)
    print(f"Total number of batches is {len(results_list)}")
    matching_quote_list = []
    ...
    for i, result in enumerate(results_list):
        ...
        matching_quote_list.extend(suppoting_text_list)

    print("\n\n", "matching_quote_list")
    print(matching_quote_list, "\n\n")

    respontheme, answerse_text = await generate_themeDescription_text(prompt, str(matching_quote_list))

    response_obj = {
        "theme": respontheme,
        "description": answerse_text,
        "supporting texts": matching_quote_list
    }
...
async def split_list_into_parts(documents, n):
    """
    Splits a list into smaller lists of size n.

    Args:
    documents (list): The list to be split.
    n (int): The size of each sublist.

    Returns:
    list: A list of sublists, each containing up to n elements.
    """
    return [documents[i:i + n] for i in range(0, len(documents), n)]
