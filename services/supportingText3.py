...
async def generate_quotes_details(projectID: str, questionId, participantId, theme, description, percentage: float = 50):
    ...
    if questionId and participantId:
        results = db.similarity_search_with_score(prompt, k=5, filter={"$and": [{"questionId": questionId}, {"participantId": participantId}]})
    else:
    ...
    # Group results into chunks of 2
    group_size = 2
    result_groups = [results[i:i + group_size] for i in range(0, len(results), group_size)]
    print("stored result in ", len(result_groups), "groups")
    all_supporting_texts = []

    async def process_result_group(result_group):
    ...
    with ThreadPoolExecutor(max_workers=math.ceil(len(results)/2)) as executor:
        ...
        futures = [
            asyncio.create_task(process_result_group(group))
    ...
    validated_list = sort_strings_by_text_occurrence(all_supporting_texts, full_context)
    print(full_context)
    print("=========================================================================")
